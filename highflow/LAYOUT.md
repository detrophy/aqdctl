...
| 0x016  | 4 x (u16, u16) | charts 1-4: source, then interval                                                       | `*-display-chart*`                                                 |
...
| 0x031  | 10 x s16 | manual calibration, correction per point, x100 (-1 -> -100)                    | `08-flow-manual-calibration-set`, `../usb-captures/10-highflow-flow-calibration` |
| 0x045  | 10 x u16 | the flow rates those corrections apply at, dL/h: 200 ... 3000 = 20 ... 300 l/h | `../usb-captures/10-highflow-flow-calibration`                                   |
| 0x292  | u16      | water quality 100 % point, 0.1 uS/cm (12.8 -> 13.3)                            | `05-water-quality-range-12.8-50.0-to-13.3-50.5`                                  |
| 0x294  | u16      | water quality 0 % point, 0.1 uS/cm (50.0 -> 50.5)                              | `05-water-quality-range-12.8-50.0-to-13.3-50.5`                                  |

- 0x02b is the internal sensor by elimination, since 0x02d is the external
  one.
- Both offsets store Aquasuite's displayed value x 100, with the same sign.
...
- The correction unit at 0x031 was not recorded; the stored value is the
  entered value x 100. Aquasuite's range is -50 to +50: both ends were written
...
| 0x05c  | 8 x 70 B | controllers, in the Octo's slot format | aqdctl's decoder       |
...
|--------|---------|----------------------------------------------------------------------------------------|-----------------------------------------------|
| 0x03   | 2 x u16 | serial number: each u16 as 5 digits, zero-padded, joined by a dash, e.g. `01234-00567` | matches Aquasuite's device information        |
...
| 0x5f   | u16     | conductivity, 0.1 uS/cm                                                                | = hwmon `fan3`                                |
...
| 0x2f-0x3e | 8 x 0x7fff           | software sensors 1-8, unset without Aquasuite |
...
NUL-padded Latin-1 (3 + 32 x 24 = 771, the checksum offset). The Octo has
42 slots.
...
- 128 x 64 pixels at 1 bit per pixel, in eight bands of 128 column bytes.
...
- `fan3_input` (conductivity) is in 0.1 uS/cm, not the nS/cm its label says.
  The water quality formula above only works in 0.1 uS/cm.
